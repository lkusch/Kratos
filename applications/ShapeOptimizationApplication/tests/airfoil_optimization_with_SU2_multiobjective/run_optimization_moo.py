...
def optimize(store_output_settings):
    # =======================================================================================================
    ...
    parameters["optimization_settings"]["output"]=store_output_settings
...
def calculate_pareto_point(outer, inner, obj):
    ...
    store_output_settings=parameters["optimization_settings"]["output"]
    #run optimization problem
    optimize(store_output_settings)
    ...
    if(parameters["optimization_settings"]["objectives"][0]["type"].GetString()=="minimization"):
        ...
        optimum=-999999
    with open("Optimization_Results/optimization_log.csv","r") as optlog:
        for line in optlog:
            cons=[]
            if(counterLog>=1):
                values=line.split(",")
                objVal=float(values[1])
                for iCons in range(0,parameters["optimization_settings"]["constraints"].size()):
                    cons.append(float(values[4+2*iCons]))
