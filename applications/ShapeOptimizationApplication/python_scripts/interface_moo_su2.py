...
def main():
    ...
    parser.add_option("-m", "--multiobjective", dest="moofile", default='',
...
def calculate_pareto_point(outer, inner, obj, project, x0, xb_low, xb_up, its, accu):
    ...
    #TODO testen ob es auch funktioniert die constraints direkt in der config zu aendern
    objec = {}
    cons= {}
...
#   cons['INEQUALITY'][this_con] = {'SIGN':def_cons[this_con]['SIGN'], 'VALUE':def_cons[this_con]['VALUE'], 'SCALE':def_cons[this_con]['SCALE']}
    ...
    #run optimization problem (make sure that return values are given)
    project=shape_optimization_moo(project, x0, xb_low, xb_up, its, accu)
        ...
        if (betterResult == 1 and inner!=0):
            for jhelp in range(0,numObj):
	            ...
	            if(signObj[jhelp]==1.0):
                        betterResult=betterResult*(project.results.FUNCTIONS[nameObj[jhelp]][ihelp]<obj[jhelp]) 
                    else: 
                        betterResult=betterResult*(project.results.FUNCTIONS[nameObj[jhelp]][ihelp]>obj[jhelp]) 
        ...
        if (betterResult == 1):
            for jhelp in range(0,numIneqCons+numEqCons):
                if(operatorCons[jhelp]=="<"):
                    betterResult = betterResult*(project.results.FUNCTIONS[nameCons[jhelp]][ihelp]<valueCons[jhelp])
                if(operatorCons[jhelp]==">"):
                    betterResult = betterResult*(project.results.FUNCTIONS[nameCons[jhelp]][ihelp]>valueCons[jhelp])
                if(operatorCons[jhelp]=="="): #TODO: insert margin (== is never true) fabs(b-a)>1E-6
                    betterResult = betterResult*(project.results.FUNCTIONS[nameCons[jhelp]][ihelp]==valueCons[jhelp])
        #store result    
        if(betterResult == 1):
    ...
    #redimensionalize objective and constraints
#    objective=[]
#    j=0
#    for i in range(0, numObj):
#	if (i==outer):
#	    optimum=optimum/(scaleObj[outer]*signObj[outer])
#	    objective.append(optimum)
#	else:
#	    constr[j]=(constr[j]/(signObj[i]*scaleObj[i]))+obj[i]
#	    objective.append(constr[j])
#	    j=j+1
...
def loop(s, itbegin, itend, border, obj, deltaObj, project, x0, xb_low, xb_up, its, accu):
	...
	if(itbegin!=itend):
            loop(s, itbegin+1, itend, border, obj, deltaObj, project, x0, xb_low, xb_up, its, accu)
        #solve optimization problem for current combination
            ...
            if(firstiteration):
            	...
            	store=calculate_pareto_point((s),(itCounter), obj,project, x0, xb_low, xb_up, its, accu);
...
def multi_objective_optimization( filename, moofile, 
    ...
    #read information on design space
    ...
    valueCons = []
    scaleCons = []
    #TODO extra Klasse fuer objective/constraint zur Uebersichtlichkeit
    ...
    if calcBorder:
        ...
        for i in range(0,numObj):
            objec=calculate_pareto_point(i,0, obj,project, x0, xb_low, xb_up, its, accu)
            border.append(objec)
	...
	for i in range(0,numObj):
	    ...
	    loop(i,0,(numObj-1), border, obj, deltaObj,project, x0, xb_low, xb_up, its, accu)
...
def shape_optimization_moo(project, x0, xb_low, xb_up, its, accun):
    sys.stdout.write('DEBL shape_optimization()\n')
    exitcode, project=SU2.opt.IPOPT(project,x0,xb_low,xb_up,its,accun)
    return project
