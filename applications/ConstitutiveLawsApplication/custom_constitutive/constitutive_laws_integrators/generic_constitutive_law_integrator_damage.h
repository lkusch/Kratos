...
class GenericConstitutiveLawIntegratorDamage
{
  public:
        ...
        )
    {
        ...
        } else { //Second region: exponential definition to consume the available Gf energy
        ...
        // const double yield_stress = r_mat_props[YIELD_STRESS];
        // const double E = r_mat_props[YOUNG_MODULUS];
        // const Matrix& strain_stress_damage_curve = r_mat_props[STRAIN_STRESS_DAMAGE_CURVE];
        // const SizeType curve_points = strain_stress_damage_curve[0].size();

        // double volumentric_fracture_energy_first_region = 0.5 * std::pow(yield_stress, 2.0) / E;
        // for (IndexType i = 1; i < curve_points; ++i) {
        //     volumentric_fracture_energy_first_region += 0.5 * (strain_stress_damage_curve[1][i-1] + strain_stress_damage_curve[1][i])
        //         * (strain_stress_damage_curve[0][i] + strain_stress_damage_curve[0][i-1]);
        //     double irreversibility_damage_check = (strain_stress_damage_curve[1][i] - strain_stress_damage_curve[1][i-1]) / (strain_stress_damage_curve[0][i] - strain_stress_damage_curve[0][i-1]);
        //     KRATOS_ERROR_IF(irreversibility_damage_check > E)<< "The defined S-E curve induces negative damage at region " << i << std::endl;
        // }
        // KRATOS_ERROR_IF(volumentric_fracture_energy_first_region > volumetric_fracture_energy) << "The Fracture Energy is too low: " << fracture_energy << std::endl;

        // double predictive_stress_end_first_region = strain_stress_damage_curve[0][curve_points] * E;
        // if (UniaxialStress < predictive_stress_end_first_region){ //First region: point-by-point definition with linear interpolation
        //     for (IndexType i = 1; i < curve_points; ++i) {
        //         if (UniaxialStress < strain_stress_damage_curve[0][i] * E){//it is no necessary to check the first value, i.e., i = 0
        //             const double current_integrated_stress = strain_stress_damage_curve[1][i-1] + (UniaxialStress / E - strain_stress_damage_curve[0][i-1])
        //                 * (strain_stress_damage_curve[1][i] - strain_stress_damage_curve[1][i-1]) / (strain_stress_damage_curve[0][i] - strain_stress_damage_curve[0][i-1]);
        //             rDamage = 1.0 - current_integrated_stress / UniaxialStress;
        //             break;
        //         }
        //     }
        // } else { //Second region: exponential definition to consume the available Gf energy
        //     const double volumentric_fracture_energy_second_region = volumetric_fracture_energy - volumentric_fracture_energy_first_region;
        //     rDamage = 1.0 - strain_stress_damage_curve[1][curve_points] / UniaxialStress * std::exp(strain_stress_damage_curve[1][curve_points]
        //         * (strain_stress_damage_curve[0][curve_points] * E - UniaxialStress) / (E * volumentric_fracture_energy_second_region));
        // }
    }
